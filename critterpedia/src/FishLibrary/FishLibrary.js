...
const FishLibrary = () => {
    ...
    // useEffect (() => {
    // let url = "https://acnhapi.com/v1/fish/1"
    // fetch(url)
    //   .then((response) => response.json())
    //   .then ((data) => setfishData(data))
    //   .catch(() => console.log("oops, error"))
    ...
    //     let fishName= critter
    //     let url = urlBase + "/" + fishName
    //     fetch(url)
    //         .then((response) => response.json())
    ...
    return (
        <div>
        <button onClick={handleClick}>Fetch all Fish</button>
        <br/>
        {/* {icon} */}
        <img className='critter-icon' src={fishData.bitterling.icon_uri} alt='icon'/>
        </div>
    )
}
