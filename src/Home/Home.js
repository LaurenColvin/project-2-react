  ...
  const iconsAvailable = availableArray.map((critter, index) => {
    ...
    return (
      <div>
        <img
          key={index}
          className="critter-icon library"
          src={icon}
          alt="icon"
          onClick={handleClick}
        />
  ...
  return (
    <div>
      ...
      <div className="search-result-container">
        <div>
          {critterAvailable == true ? (
            <div className="search-result">
              <h4 className="details-name">
                {currentCritter.name["name-USen"]}
              </h4>
              <div className="critter-details">
                <img
                  className="critter-icon details"
                  src={currentCritter["icon_uri"]}
                  alt="icon"
                />
                <div className="details-container">
                  {critterType !== "sea" ? (
                    <h3 className="search-details">
                      Rarity: <br />
                      <span>{currentCritter.availability.rarity}</span>
                    </h3>
                  ) : (
                    <h3 className="search-details">
                      Shadow size: <br />
                      <span>{currentCritter.shadow}</span>
                    </h3>
                  )}
                  {critterType !== "sea" ? (
                    <h3 className="search-details">
                      Find it:
                      <br /> <span>{currentCritter.availability.location}</span>
                    </h3>
                  ) : (
                    <h3 className="search-details">
                      Speed: <br />
                      <span>{currentCritter.speed}</span>
                    </h3>
                  )}
                </div>
              </div>
              <div>
                {caughtAlert == true ? (
                  <div>
                    <h5 className="alert">
                      This critter has been added to your caught library!
                    </h5>
                  </div>
                ) : (
                  <div>
                    <h5 onClick={handleClick}>
                      Click to add this critter to your caught library!
                    </h5>
                  </div>
                )}
              </div>
            ...
            <div>
              {critterAvailable == false ? (
                <h4 className="search-by-month">
                  Search for critters by month!
                </h4>
              ) : (
                <h4>Click on the icon for more info!</h4>
              )}
            </div>
          )}
        </div>
      </div>
      <div className="icon-list">{iconsAvailable}</div>
    </div>
  );
};
